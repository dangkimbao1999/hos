...
mock.module("@/lib/supabase/packages", () => ({ listCartItems: async () => [] }));

let profileToReturn: Profile | null = { id: "org-1", role: "organizer", full_name: "Test Organizer" } as Profile;
...
describe("OrganizerLayout", () => {
  it("fetches categories/cities and passes them to AppShell", async () => {
    profileToReturn = { id: "org-1", role: "organizer", full_name: "Test Organizer" } as Profile;
    const jsx = await OrganizerLayout({ children: <span>child</span> });
    render(jsx);
  ...
  it("redirects to sign-in when there is no signed-in profile", async () => {
    ...
    await expect(OrganizerLayout({ children: <span /> })).rejects.toThrow("REDIRECT:/sign-in");
  });
  ...
  it("redirects to the profile's own role home when role doesn't match", async () => {
    ...
    await expect(OrganizerLayout({ children: <span /> })).rejects.toThrow("REDIRECT:/talent");
  });
});
