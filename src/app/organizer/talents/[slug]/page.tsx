...
import { CalendarDays, CheckCircle2, ImageIcon, Star } from "lucide-react";
    ...
    <div className="flex flex-col gap-6 py-8">
      <div className="relative flex h-[280px] w-full flex-col justify-end overflow-hidden rounded-md">
        ...
        <div className="relative flex flex-col gap-1 p-8">
          <h1 className="text-4xl font-bold tracking-[-0.03em] text-white">{talent.name}</h1>
          <span className="text-sm text-white/60">{talent.category}</span>
        </div>
      </div>
      ...
      <div className="flex gap-8">
        <div className="flex min-w-0 flex-1 flex-col gap-8">
          {tab === "Overview" && (
            <>
              <RatingReviewCard talent={talent} />
                    ...
                    <div className="flex items-center gap-0.5">
                      {Array.from({ length: 5 }).map((_, s) => (
                        <Star
                          ...
                          className={cn(
                            "size-3.5",
                            s < review.stars ? "fill-primary text-primary" : "text-white/20"
                          )}
                        />
                      ))}
                    </div>
