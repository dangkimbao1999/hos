...
mock.module("@/lib/supabase/quotations", () => ({
  listQuotationsForOrganizerPage: async (organizerId: string, page: number) => {
...
import OrganizerQuotationsPage from "@/app/organizer/account/quotations/page";

function makeQuotation(id: string): QuotationWithNames {
  return { id } as unknown as QuotationWithNames;
}

describe("OrganizerQuotationsPage", () => {
  it("fetches the current page of quotations, passing them to QuotationsContent", async () => {
    ...
    const jsx = await OrganizerQuotationsPage({ searchParams: Promise.resolve({}) });
    render(jsx);
  ...
  it("resolves the page number from ?page=", async () => {
    ...
    await render(await OrganizerQuotationsPage({ searchParams: Promise.resolve({ page: "3" }) }));
    expect(pageArgs).toEqual({ organizerId: "org-1", page: 3 });
  });
});
