...
import TalentOrderDetailPage from "@/app/talent/account/orders/[bookingId]/page";
...
describe("TalentOrderDetailPage", () => {
  it("renders OrderDetailContent with the fetched booking, scoped to the talent role", async () => {
    bookingResult = makeBooking();
    const jsx = await TalentOrderDetailPage({ params: Promise.resolve({ bookingId: "booking-1" }) });
    render(jsx);
    expect(screen.getByTestId("content")).toHaveTextContent("talent:Acoustic Set");
  });

  it("calls notFound when the booking doesn't exist (or isn't visible to this viewer)", async () => {
    bookingResult = null;
    await expect(
      TalentOrderDetailPage({ params: Promise.resolve({ bookingId: "missing" }) })
